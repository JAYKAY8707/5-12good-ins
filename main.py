...
def query_page():
    ...
    # Get unique doctors and specialties from JSON
    doctors = list(set(d['doctor'] for d in data['doctors_specialties']))
    specialties = list(set(d['specialty'] for d in data['doctors_specialties']))
    specialties = [
    ...
    insurances = []  # We'll handle insurance separately if needed
        ...
        for entry in data['doctors_specialties']:
            if entry['doctor'] == doctor_query:
                matching_doctors.append({
                    ...
                    'insurances': 'No insurances listed'
                })
                break
    
    elif specialty_query:
        for entry in data['doctors_specialties']:
            # Split specialties by comma and strip whitespace
            doc_specialties = [s.strip() for s in entry['specialty'].split(',')]
            if specialty_query in doc_specialties:
                matching_doctors.append({
                    'name': entry['doctor'],
                    'specialties': entry['specialty'],
                    'insurances': 'No insurances listed'
